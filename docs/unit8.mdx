...
Let's do an example with the [Anteater API](https://anteaterapi.com) (formerly the PeterPortal API).
This API was created by ICSSC and is in use by applications like AntAlmanac and PeterPlate.
...
- `https://anteaterapi.com` - This means we want to make a request to this server. Your internet will use this to find the server.
...
<!-- docasaurus tries to pull a variable called id -->
[https://anteaterapi.com/reference#tag/courses/GET/v2/rest/courses/\{id\}](https://anteaterapi.com/reference#tag/courses/GET/v2/rest/courses/{id})
...
To make a request to an API in JavaScript, we use the `fetch` function.
Fetch is a function that takes 2 arguments. The url and a configuration object.

```jsx
const url = "https://anteaterapi.com/v2/rest/ping";
...
For the PeterPortal API, the documentation is found in its [GraphQL Playground](https://anteaterapi.com/v2/graphql), under the `docs` tab on the right.
...
One of these is the `instructors` which is an array of Instructor types.
...
It is being used by all of our our current projects.

Check it out here: [Anteater API Docs](https://anteaterapi.com)
...
-  Make a Request to the Anteater API using REST with Fetch
...
### Submission
Complete the [Google Form](https://forms.gle/JFQxjth5imZGHqHM7)
<iframe src="https://docs.google.com/forms/d/e/1FAIpQLScX_0IVFlKn0_Qvl8GhLyTw2k02vggutf9YqOBrCnWSnL8hnA/viewform?embedded=true" width="100%" height="1400" frameborder="0" marginheight="0" marginwidth="0">Loading…</iframe>
