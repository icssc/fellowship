...
We haven't gone much into depth about JavaScript, but good thing is it's very similar to many languages you might have used before (Python, Java). Here's a quick overview of the basics.
...
Variables are dynamically-typed, meaning, they don't have a type (like in Python). Undefined variables have the value `undefined`. We use `let` and `const` to define variables.
...
1. `useState` is a Hook, that we call inside a function Component to give it a local state. A state lets us preserve these values after the function call ends. 
    ...
    2. `useState` returns a pair: the current state value `count`, and a function that lets you update the state to a new value `setCount`
        1. We use syntax called “array destructuring”. Also in python, this lets us set variables to individual items in an array.
2. For the function component we return the markup for the component. Notice the use of curly brackets around our JavaScript.
...
Now that node is installed, it also comes with `npm` a package manager for node. `npx` is a package runner that is pre-bundled with `npm`. We use `npx` to create our first react app. 
...
![Untitled](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/8f6ba7f9-1638-40d0-81e7-5d2d83c07256/Untitled.png)
...
- [https://learnjavascript.online/app.html](https://learnjavascript.online/app.html) (They have a JavaScript one too!)